...
const GenerateImages = () => {
  ...
  const imageStyle = ['Realistic', 'Ghibli Style', 'Anime', 'Cartoon', 'Fantasy', 'Sci-Fi', 'Portrait', '3D']
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      const prompt = `Generate a ${selectedCategory} image of ${input}`
  ...
  return (
    ...
    flex-wrap gap-4 text-slate-700'>
      ...
      border border-gray-200'>
        ...
        text-sm rounded-md border border-gray-300' placeholder='Describe what you want to see in the image..' required />

        <p className='mt-4 text-sm font-medium'>Style</p>
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {imageStyle.map((item) => (
            <span onClick={() => setSelectedCategory(item)} className={`text-xs px-4 py-1 border rounded-full
            cursor-pointer ${selectedCategory === item ? 'bg-green-50 text-green-700' : 'text-gray-500 border-gray-300'}`} key={item}>{item}</span>
          ))}
        </div>
        ...
        <div className='my-6 flex items-center gap-2'>
          <label className='relative cursor-pointer'>
            ...
            bg-white rounded-full transition peer-checked:translate-x-4'></span>
          </label>
      ...
      flex-col border border-gray-200 min-h-96'>
        ...
        {!content ? (
          ...
          text-gray-500'>
              ...
              <p>Describe the image and click "Generate Image" to see the result</p>
